 ...
 *
 * Phase 2 scope: spawn/handshake/tool-cache/dispatch/disposal. Crash
 * restart policy, config-change reconciliation, roots, and the
 * instructions prompt block are Phase 3.
 *
...
import { loadMcpServers, McpServerConfig } from './config';
...
export class McpServerManager {
  ...
  /** Captured at handshake; the prompt block that consumes it is Phase 3. */
  ...
  dispose(): void {
    ...
    for (const entry of this.servers.values()) {
      ...
      // close() SIGTERMs the child via the stdio transport. A server still
      // mid-handshake has no client yet — close its transport directly.
  ...
  private async startServer(entry: ManagedServer): Promise<void> {
    ...
    try {
      ...
      client = new Client(
        ...
        // No sampling/elicitation — declined by omission. Roots lands in
        // Phase 3; declaring it without a roots/list handler would hand
        // servers a method-not-found.
        { capabilities: {} }
      );

...
- **Webview re-resolve spawns a spurious new session instead of restoring the active one (found 2026-08-03, dev-host).** Mid-test the webview reloaded (19:07:42 in the trace export) and the extension created a fresh "New Chat" (`8d5aeb99`) rather than restoring the session in use (`27a7f659`) — the user had to switch back by hand. Cold activation does this correctly (`loadCurrentSession: restored … from shared-key`), so the re-resolve path in `resolveWebviewView` diverges from cold start. Reproduce by closing/reopening the chat panel mid-session, then diff the two paths.

...
2. **Composer autocomplete (Medium) — design accepted 2026-08-04, goes before MCP.** Typed invocation in the input area: `/` commands, `@` files (async, reuses `searchFiles`), `:` emoji (bundled gemoji pairs, ≥2-char query, boundary-guarded). One new webview actor + provider registry; textarea stays a plain textarea (accept converts to chips/commands/inserted text — no contenteditable). Emoji-first build order proves the overlay with zero async. This is the landing surface for MCP prompts (`/`) and resources (`@`). Plan: [docs/plans/composer-autocomplete.md](docs/plans/composer-autocomplete.md); decisions: [ADR 0015](docs/architecture/decisions/0015-composer-autocomplete-typed-invocation.md).

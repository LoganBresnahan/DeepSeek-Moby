...
- **`thinkingMode: 'disabled'` is a no-op on custom models (found 2026-08-03, dev-host).** `applyThinkingMode` returns early on `!caps.sendThinkingParam` ([deepseekClient.ts:351](src/deepseekClient.ts#L351)), so the router's forced non-thinking — which exists precisely because reasoning latency dominated Phase 1 sub-call cost — never reaches a custom-model backend. The `image-describe` route on Kimi burned `reasoning=349 chars` and took **30s**, blocking the turn behind "Analyzing image…". No safe generic fix: there is no portable OpenAI-compatible "disable thinking" param, and inventing one risks a 400. Options are a per-model `disableThinkingParam` field naming the provider's own knob, or documenting "pick a fast non-reasoning vision model for this role". **Note the earlier guess that image payload size caused the latency was wrong** — the payload was only 21KB; the call itself took 29,971ms of the 29,976ms route.
- **The request timeout is inert on streaming calls, and `chat()` ignores its abort signal (found 2026-08-03, reading the code after the timeout fix).** [httpClient.ts:80](src/utils/httpClient.ts#L80) does `config?.signal ?? controller.signal` — when a caller supplies a signal, the timeout controller's `abort()` has **no listener**, so the timeout silently does nothing. `streamChat` passes `signal: options?.signal` ([deepseekClient.ts:558](src/deepseekClient.ts#L558)), so **streaming requests have no timeout at all** and a hung stream only ends when the user hits Stop. Conversely `chat()` ([deepseekClient.ts:444](src/deepseekClient.ts#L444)) passes **no** signal despite accepting `options.signal`, so it *does* time out (this is what killed the iteration-3 turn) but **cannot be cancelled by Stop**. Both halves are wrong in opposite directions. Fix: combine the two signals (`AbortSignal.any`) rather than choosing one, and forward `options.signal` from `chat()`. Interacts with ADR 0008's interrupt teardown — check that path before changing it.
- **Token over-count on image turns, ~2.4× (found 2026-08-02, dev-host).** `[TokenCV] ours=7,445 api=3,040 delta=-144.9%` on a turn with an image, while the *streaming* call in the same turn was fine (`ours=3,295 api=3,461`, +4.8%). Same turn, so `countRequestTokens` ([deepseekClient.ts:906](src/deepseekClient.ts#L906)) is seeing ~4,000 tokens the streaming path doesn't — and `[Context]` reported only 2,938 for the same request, so ContextBuilder disagrees with it too. The delta size closely matches a base64 data URI at the calibrated estimation ratio, so the leading hypothesis is that the image data URI reaches the counted payload on the non-streaming `runToolLoop` probe. **Unverified** — the API prompt counts stay normal, so nothing extra is actually being *sent*; this is a metrics defect. It still matters because the same count feeds the context-budget guard, which could drop history prematurely in an image-heavy session.
- **`supportsTemperature` can't express "must be exactly N" (found 2026-08-02, dev-host).** Kimi rejects any temperature but 1 (`invalid temperature: only 1 is allowed for this model`), and the axis is a boolean, so the only workaround is setting the global `moby.temperature` to 1 — which then applies to every model. Needs either a `temperatureFixedValue`/allowed-range field on `ModelCapabilities` or a per-model temperature override.
...
1. **Subagent Phase 2 — `image-describe` (Medium).** Subagent routing scaffolding shipped in 0.6.x. **Plan phases 0 and 1 shipped 2026-08-02** — attachment persistence + replay (ADR 0014) and foundations (router `buildUserContent` hook, attachment type threading, webview image capture, `acceptsImages` capability). **Phases 2–3 also shipped 2026-08-02:** the `image-describe` role ([imageDescribe.ts](src/subagents/roles/imageDescribe.ts)) + orchestrator routing. Digests resolve **before** `recordUserMessage` (plan §7 option (i)) so they persist on the attachment and replay by construction — injecting live would have reintroduced the reload-drift ADR 0014 fixed. Guard rails: images never touch the text `--- Attached Files ---` formatter, a failed route always becomes a *named* placeholder (never silence), and `assertNoArrayContent` at the single `contextMessages` choke point keeps `image_url` blocks away from the text-only main model. Remaining: phase 4 (512px archive thumbnail + lazy blob fetch), 5 (transcript render), 6 (backlog discharge). **Needs a vision backend to exercise** — see M37. **Phase 1b — drag-and-drop attach onto the input box — also shipped 2026-08-02** ([§8](docs/plans/image-describe-subagent.md#8-drag-and-drop-attach)): every drop attaches (no origin routing, no modifier keys), OS drops read in the webview, Explorer drops round-trip through `requestDroppedFiles`/`droppedFileContents`. The document-level `preventDefault` guard is load-bearing — without it an off-target drop navigates the webview away and blanks the chat. **Interim state worth knowing:** the file picker now accepts images and persists them as binary blobs, but nothing routes them yet — an attached image is stored and shown as a chip while the model is told nothing about it until phase 3 lands. Vision via digest routing — auto-digest on image attach, backend-agnostic (user-configured vision model, no bundled default), 512px WebP thumbnail + digest persisted as content-addressed blobs beside the events table. Decision-locked plan: [docs/plans/image-describe-subagent.md](docs/plans/image-describe-subagent.md) (supersedes the Phase 2 section of [subagents.md](docs/plans/subagents.md)); revised 2026-08-02 with decisions 4–6. **Start at Phase 0** — attachment persistence + replay, which fixes the Active Bug above and ships standalone value for text attachments before any image code lands.

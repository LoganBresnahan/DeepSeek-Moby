...
- **Image content poisons the estimation counter's calibration (found 2026-08-03, dev-host — mechanism confirmed).** `extractText` counts an `image_url` part as the literal string `'[image]'` ([contextBuilder.ts:43](src/context/contextBuilder.ts#L43)) — ~2 tokens for something the API bills at 1,000+. On the first `image-describe` route: `[TokenCV] ours=325 api=1,133`, and the estimation counter then *calibrated on that sample* → `ratio=0.0499`, roughly 10× off. Every later estimate from that client is garbage. Subagent clients are per-modelId instances, so this corrupts the vision client specifically, but the same path runs for any image-bearing request. **Fix direction:** skip calibration when the counted payload contains image parts — the sample is unmodellable by a text tokenizer, so feeding it to the calibrator wrecks a counter that was otherwise fine. Modelling image tokens properly is the larger follow-up.
- **Request timeout is a hardcoded 60s (found 2026-08-03, dev-host).** [httpClient.ts:41](src/utils/httpClient.ts#L41) — `config.timeout ?? 60000`, with no setting. A Kimi turn died at iteration 3 on `Request timeout` after two successful tool iterations, losing the turn. Slow reasoning providers routinely exceed this: the `image-describe` digest alone took **30s** on the same run. Needs a `moby.requestTimeoutMs` setting, and arguably a longer default for non-DeepSeek endpoints.
- **Image digest latency blocks the turn, ~30s (found 2026-08-03, dev-host).** `[subagent.route] image-describe … durationMs=29976` with `inputBytes=21646`. Routing is awaited before `recordUserMessage`, so the user stares at "Analyzing image…" for half a minute before the main turn even starts. The `/verify` roster's "attach → indicator → digest" step passes, but the *feel* is bad. Options: shrink the image sent to the sub (currently ~1024px), or overlap routing with the main request's system-prompt build.

...
4. **Registry booleans that need values** — `supportsTemperature`, `sendThinkingParam`, `reasoningTokens` are all too coarse for real custom models (Kimi trips all three). Ship-blocking only insofar as (3) is; otherwise a fast-follow. **Add streaming-tool-call gating to the list (2026-08-03, dev-host):** kimi-as-main ran the legacy non-streaming probe *plus* a streaming pass — one image turn cost 41.9s wall (22.6s probe whose output was regenerated by the 17.4s stream), paying the reasoning tax twice. The M27 duplicate-generation fix never reached custom models.
...
- **Calibration guard bypassed on the main-model tool probe (found 2026-08-03, dev-host — extends the over-count bug below).** `[TokenCV] ours=10,187 api=3,012 delta=-238.2%` on kimi-k3's non-streaming `runToolLoop` probe, and the sample **calibrated** (`ratio 0.7642 → 0.6226`) — the `hasImageContent` skip fired on both subagent digest calls in the same run but missed this path. Confirms the hypothesis below: the API prompt count stayed normal, so the data URI is *counted*, never *sent*. Also suspicious: both main-model requests logged `hasImages: true` / `(with images)` — v4-flash-thinking (text-only) got a 200, so the wire was clean, meaning that flag is either derived from attachment metadata (misleading logging) or a second sighting of the counted-payload leak. Find where the probe's counted payload picks up the attachment body and extend the guard there.

...
| B20 | Platform-specific VSIX packaging | **Fixed** |
| B21 | Input area layout and expand/collapse | **Open** | Multiple issues: (1) Attachment chips overflow and cover chat content. (2) Expanded textarea covers chat content instead of pushing it up. (3) Need a toggle button (top-right) to manually expand/collapse the input between min-height and full content height. Fix: move input from absolute positioning back to flex layout so growth pushes chat up. Add collapse/expand toggle arrow. Solves both attachment overflow and textarea coverage | Currently shipping all 6 SQLCipher prebuilds (12MB) to every user. `vsce` supports `--target` flag to publish platform-specific VSIXs (linux-x64, darwin-arm64, win32-x64, etc.) — each user downloads only their ~2MB prebuild instead of all 6. Requires publishing 6 separate VSIXs per release but halves the download size |

...
- **Decision gate — RESOLVED 2026-08-11: keep and fix the built-in path.** Full reasoning in [ADR 0016](../architecture/decisions/0016-mcp-client-integration.md) § "Built-in LSP deprecation". Short version: the two serve different populations — the built-ins need only a VS Code language extension (everyone has one), pharos needs standalone LSP binaries on `PATH` (few do). The probe's flakiness was under-investment, not a wrong architecture, and it was fixed the same day along with the tool-surface gap (added `hover` + `get_diagnostics`) and a worse latent bug the investigation surfaced: the availability check gated the LSP **tools themselves**, so a mis-marked workspace lost them entirely and no tool result could ever correct the map. Both surfaces now coexist. M44 S6 stays worth walking as a quality comparison, but it is no longer a survival gate.
...
3. ~~Should the status command be a full webview surface or just an output-channel dump? Leaning dump — parity with LSP availability's log-based visibility.~~ **Resolved 2026-08-11 — a QuickPick, one step up from the dump, that also toggles `enabled`.** See [ADR 0016](../architecture/decisions/0016-mcp-client-integration.md) decision 14. The dump was too little: MCP failure is invisible (tools just leave the array), and the only status surface shipped was `refreshMcpServers`, which restarts everything to report on it. A modal was too much for the value. The checkbox writes settings rather than runtime state so the manager keeps one source of truth.

...
- [ ] **test-harness-inmemory-and-fixture-server** (medium, moderate) — `InMemoryTransport` unit tier + 3-tool fixture stdio server (echo / slow-tool / error-tool) for child-process lifecycle and `listChanged` (pharos declares `listChanged: false`, so the fixture is the only coverage of that path). **Make the 30s timeout injectable before writing the slow-tool spec** or CI inherits a 30s sleep — and make `RESTART_BACKOFF_MS` injectable for the same reason.

  **Two crash-policy scenarios are already proven against real child processes** (throwaway harness, 2026-08-07 — deliberately not kept in the unit tier, where an 18s test doesn't belong): a **handshake-then-exit** server went ready → crash → restart 1 → crash → restart 2 → crash → `failed` with `restartAttempts=2` and no pending timer (the budget correctly never reset), and a **bogus command** produced `spawn … ENOENT` with `everReady=false`, zero attempts, no timer. `restartAll()` revived the first back to `ready`. Fold both into the fixture tier with injected backoff so they run in milliseconds.

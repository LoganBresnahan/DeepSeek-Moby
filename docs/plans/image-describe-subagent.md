...
- [x] **2. Role module + settings filter** *(opus)* — **SHIPPED 2026-08-02.** The `acceptsImages` gate landed as *enforcement in the router* rather than a dropdown filter: there is no image-describe dropdown to filter (the setting is JSON-only), so filtering a nonexistent UI would have been dead code. A role now declares `requiresImageSupport` and the router refuses a model without `acceptsImages`.
  ...
  - [x] `accepts-images-capability-filter` (low/mechanical) — **re-scoped to router enforcement** (`requiresImageSupport` on the role + `acceptsImages` check in `route()`). A dropdown filter can reuse the same capability when an image-describe UI is built.

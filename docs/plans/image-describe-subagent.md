...
**Status:** Phases 0, 1, 1b, 2, 3 shipped 2026-08-02; phases 4 and 5 shipped 2026-08-03. Remaining: 6 (backlog discharge).
...
- **Blob-fetch batching / size guard** (phase-5 adversarial review, 2026-08-03). `requestAttachmentBlob` is one round-trip per unique blobId with dedupe + a session cache, so normal sessions (~25–40KB archive blobs) are fine. The tail risk is **legacy pre-phase-4 rows**, which stored the full ~1024px rendition (up to 1.5MB) — fast scrolling through an image-heavy legacy transcript issues N independent multi-MB replies. If that shows up in practice: a batched `requestAttachmentBlobs([...])` plus a per-reply size cap. Data-gated; not worth building against hypothetical transcripts.
...
- **Reasoning-model latency on custom backends (confirmed 2026-08-03).** The router forces `thinkingMode:'disabled'` on every sub call, but `applyThinkingMode` returns early on `!caps.sendThinkingParam`, so that force **never reaches a custom model**. A Kimi `image-describe` route took **30s**, burning `reasoning=349 chars` — the exact tax the router exists to avoid. Not fixable inside this plan (there is no portable OpenAI-compatible disable-thinking param); tracked in CLAUDE.md § Active Bugs. Practical guidance until then: **point this role at a fast non-reasoning vision model**, since the digest blocks the turn.
...
- ~~**jsonMode on the VL backend.**~~ **Closed 2026-08-03.** Router forces `jsonMode:true`; a VL model ignoring `response_format` would have meant `parse-fail` → placeholder. Two Kimi routes validated `ok`, so the lenient parse + strict prompt instruction suffice and the per-role opt-out was not built. The router now labels how each response parsed (`jsonRecovery`) so a future backend that *does* ignore `response_format` announces itself instead of hiding behind a successful recovery. Full reasoning: [the jsonMode question, answered](#the-jsonmode-question-answered-2026-08-03).
...
- [x] **4. Thumbnail + digest persistence** *(opus)* — **SHIPPED 2026-08-03.** Own phase because its failure mode (full-res bytes reaching the table) is **silent**. Much thinner now that Phase 0 owns storage and replay. Gate: persisted-attachment-size unit test + `Moby: Export Turn as JSON` during `/verify`.
  - [x] `thumbnail-digest-persistence` (low/mechanical after Phase 0, risk: medium) — put the webview-made 512px WebP into `attachment_blobs`, reference it + the digest from `UserMessageEvent.attachments` (shape already exists in [EventTypes.ts:31](../../src/events/EventTypes.ts#L31)); full-res bytes never reach the table. **Entry gate: resolve the digest-ordering decision in [§7](#7-attachment-replay-the-correctness-prerequisite)** — record-after-routing vs appended digest row — before writing this slice.
  - [x] `digest-replay-coverage` (landed early, with Phase 3) (low/mechanical) — extend `replay-equivalence-tests` to an image turn: the digest block must rebuild identically on reload, and a `routed:false` placeholder must replay as the placeholder rather than silently vanishing.
- [x] **5. Transcript thumbnail render on reload** *(fable + adversarial verify — the only slice that warrants it)* — **SHIPPED 2026-08-03.** ADR 0003 hydration-invariant work: restore-path render must not double what the live path drew; a plausible-but-wrong version passes live testing and breaks only on reload/fork. **Implementation deviation from the §6 sketch, deliberate:** no projector segment type. User-turn attachments are not structural events — they live on the `user_message` event — and the projector only serves assistant turns, so a "thumbnail segment" would have created a restore-only render path, exactly the divergence this phase warns against. Instead the existing user-turn `files` channel was upgraded to rich `attachments` metadata on `TurnData`, and **one** render site ([MessageTurnActor.renderTextSegment](../../media/actors/turn/MessageTurnActor.ts)) draws chips for live and restore alike: live sends carry the archive rendition's data URL from the composer; restored turns carry `{blobId, w, h}` and render a dimension-reserved placeholder until bytes arrive. The assistant-`drawing` no-op is untouched (drawings restore via their own path; nothing user-visible regressed). `/verify` must exercise reload, session switch, and fork explicitly, then `/shipshape`.
  - [x] `transcript-thumbnail-render` (high/hard-reasoning) — `TurnAttachment` on the webview's turn model; `getSessionRichHistory` projects persisted attachments → render metadata (images ship `blobId`/`w`/`h`, text ships name only — never bodies, never the digest); live/restore consistency by shared render path.
  - [x] `lazy-blob-fetch` (medium/moderate) — hydration ships `{blobId, w, h}` only; `VirtualListActor.bindActorToTurn` (the visibility trigger) requests bytes via `requestAttachmentBlob` → `attachmentBlob`, deduped per blobId with a null-cached miss path so a GC'd blob is never re-requested. Reserved `width`/`height` attributes mean a late-arriving blob cannot reflow the list. Both message types registered with the [protocol drift detector](../../tests/integration/protocol-drift.test.ts); the scanner regex was widened to see optional-chained `postMessage?.()` sends, which surfaced and discharged the pre-existing `poolWarning` orphan (handler added). **Adversarial verify ran (two independent reviewers, 2026-08-03) and caught five real defects, all fixed + test-pinned:** (1) image-only turns (no caption — the primary use case) hid their own thumbnail via the empty-content container hide; (2) attribute-context injection through attachment names (`escapeHtml` never escaped quotes — new `escapeAttr`, also applied to the pre-existing `data-file-path`/`data-diff-id` sink); (3) a non-decodable image data URI persisted the literal string under an image mime — a permanently-broken thumbnail (now metadata-only persist + webview name-tag fallback); (4) a throw or missing view in `sendAttachmentBlob` silently wedged the blobId in the webview's pending set (now replies unconditionally, and refuses non-image blobs so text bodies stay unfetchable); (5) legacy dimensionless rows reflowed on blob arrival under `height:auto` (fallback box now CSS-locked). Reviewers also flagged the missing webview CSP (filed in CLAUDE.md Planned Work) and blob-fetch batching (deferred below).
- [x] **6. Manual-test backlog + discharge** *(opus)* — **code-side complete 2026-08-03; discharge is dev-host-gated and still open.** Also the trigger for the jsonMode-on-VL-backend empirical check (answered below).
  - [x] `manual-test-backlog-entries` (low/mechanical) — the six scenarios from [Testing](#testing) all have homes, written **incrementally as each phase landed** rather than batched here: digest-reaches-main → M37 S2; no-backend placeholder → M37 S1; oversized image → M35 step 7; multiple images → M37 S6; reload restores digest **and** thumbnail → M37 S5 + M39; text attachment survives reload → M34 step 3. Reconciled 2026-08-03; no gaps found, nothing added.
  - [x] `json-recovery-instrumentation` (low/mechanical) — **added during build.** The empirical question below could not actually be answered from a trace: `tolerantJsonParse` recovered a fence *silently*, so a wrapped response and a clean one both logged `validationResult: 'ok'`. The router now records `jsonRecovery: 'none' | 'fence' | 'brace-scan'` on the success span and logs an info line when recovery fires.
  - [ ] **Discharge (dev-host, user-run).** `/verify` must walk M34–M39 and delete or annotate each. Not something `/shipshape` can close — see the verification roster.

### The jsonMode question, answered (2026-08-03)

Plan risk *"jsonMode on the VL backend"* and sequencing risk 4 asked whether a vision backend would honour `response_format`, with a per-role `jsonMode` opt-out held in reserve.

**Evidence:** two `image-describe` routes against Kimi (`kimi-k3`, Moonshot) in a dev-host run — `inputBytes` 36,451 and 21,646, `digestBytes` 975 and 1,396, **both `validationResult: 'ok'`**. No `parse-fail`, no `schema-fail`. Digest quality was good enough to answer questions from (an accurate unprompted description of a stamp-style lighthouse illustration).

**Conclusion: the contingency is not needed.** Do not build the per-role opt-out on current evidence.

**Honest limit:** that run predates the `jsonRecovery` field, so it cannot distinguish "Kimi emitted clean JSON" from "Kimi fenced it and `tolerantJsonParse` recovered transparently". Either way the pipeline held, which is what the risk was about — but the *next* run answers it precisely, and a backend that shows persistent `fence`/`brace-scan` recovery is the signal to revisit. One backend is also not a population: a second VL backend (SiliconFlow `deepseek-vl2`) is worth one route before treating this as settled.
...
4. ~~**jsonMode on VL backends** is only testable empirically in phase 6; if it fails, the per-role jsonMode opt-out contingency touches the phase-2 role module.~~ **Resolved 2026-08-03** — two clean Kimi routes, contingency not built. See [the jsonMode question, answered](#the-jsonmode-question-answered-2026-08-03).

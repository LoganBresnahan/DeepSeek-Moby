...
3. ~~**Kimi temperature pin.**~~ **DISCHARGED 2026-08-04 (dev-host, real Moonshot):** with the global temperature at 0.5 and `temperatureFixedValue: 1` on the entry, the send succeeded — Moonshot rejects any non-1 temperature, so the wire can only have carried the pinned value. Future runs are explicit: the client now logs `[Temperature] Pinned to N for <model> (global setting is M)` whenever the pin overrides a differing global.

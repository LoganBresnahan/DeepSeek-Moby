...
| B25 | Chained shell commands miss file watcher deletions | **Partial Fix** | File watcher may miss deletion events on WSL2 due to inotify bridging latency. Mitigated by `commandsDeleteFiles()` which detects `rm`/`unlink` patterns from the command string to prevent false auto-continuation. The "Modified Files" dropdown may still not appear if the watcher misses the event |
| B26 | History restore dropdown positioning differs from live | **Fixed** | `projectFull` didn't break the text flow on `file-modified` events, so subsequent text appended above the dropdown. Fix: `file-modified` now finalizes the current text segment (like `thinking-start` does), forcing subsequent text into a new segment below the dropdown |

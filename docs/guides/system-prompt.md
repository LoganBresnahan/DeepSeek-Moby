...
> Decision records: [ADR 0007 — System-prompt temporal grounding](../architecture/decisions/0007-system-prompt-temporal-grounding.md), [ADR 0013 — Temporal grounding II (data-seeding clause)](../architecture/decisions/0013-temporal-grounding-data-seeding.md).
...
- **The seeding clause is primacy, not recency (ADR 0013).** A traced run built an app and populated it with *stale* World-Cup data, having called `web_search` **zero** times — the miss happened at iteration 1 with the directive already fresh at the top of the prompt. So it was a task-**classification** failure ("seed app data" filed as coding, not lookup), not salience decay over a long turn. An adversarial design pass rejected re-pinning the date at recency as the fix: the date is **static** within a turn, so re-pinning it just re-states wording the model already ignored at primacy while diluting the *dynamic* active-plan reminder ([ADR 0009](../architecture/decisions/0009-active-plan-recency-pinning.md)) that shares the recency edge. The fix is the wording above. A narrowly-gated **behavioral** recency pin (fire only after the model writes fact-bearing data without searching) is a documented 0013 follow-up, deferred until trace data shows it is needed.
...
- [ADR 0013](../architecture/decisions/0013-temporal-grounding-data-seeding.md) — the data-seeding clause that reframes "populate this with real-world facts" as a time-sensitive lookup, and the reasoning for keeping the fix at primacy rather than re-pinning the date at recency.

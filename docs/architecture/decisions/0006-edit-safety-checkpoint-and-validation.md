...
Auto mode **stays Auto** — it never demotes to per-edit approval (Alternative G). The repair loop in layer 6 is bounded **per file by repeated-failure detection**. Two terminal outcomes:

- **A file won't converge** — the halt budget is tracked **per file**, keyed on the file path, and counts only *consecutive failures with the same normalized error set*. A file is `stuck` (→ **halt the turn**) when it reverts with the **same error `maxRepairAttempts` times in a row**. Two properties fall out of this:
  - **Independent failures don't accumulate.** Three different files each failing once is normal work, not a stuck loop — each keeps its own streak of 1, so the turn does not halt. (A flat per-turn counter would have wrongly halted at three.)
  - **Progress earns a fresh budget.** A file whose error set *changes* between attempts (fixed bug A, now hitting bug B) resets its streak to 1 — the model is making headway, so it isn't punished. Only a file *reproducing the identical failure* is halting. Because `normalizeErrors` keeps the file path in each signature, different files inherently produce different sets, so "same error" is implicitly "same file, same problem."

  On halt: leave every file at last-good, emit a clear terminal status naming the stuck file ("`X` kept breaking `<check>` with the same error after N attempts — reverted to last-good; here's what failed: …"), and stop. Control returns to the user with no covert mode switch. Files *other* files successfully repaired this turn stay committed. (A simple cycle — error A → B → A → B on one file — defeats consecutive-same detection; the turn's overall iteration cap is the ultimate backstop. Set-of-seen-errors cycle detection is a possible refinement.)
...
| `moby.editSafety.maxRepairAttempts` | `3` | Per-file budget: how many times a single file may revert with the **same** error in a row before the turn halts (a changing error resets it; other files are independent). With this per-file/same-error semantics, `3` is already lenient — raise it for extra rope on slow-converging fixes. |
...
- **Reverted-file "Modified Files" UX (known gap; behaviour decided, not yet built).** The auto path emits a file as modified *before* the settle point (`notifyAutoAppliedFilesChanged` reads an append-only `resolvedDiffs`; [diffManager.ts:1945](../../../src/providers/diffManager.ts#L1945)), so after a regression revert the dropdown still lists the now-reverted file. The content *is* correctly restored and the model *is* told via the feedback message — only the UI signal is stale. **Decided behaviour:**
  - *Mid-turn revert that the model then repairs* → **invisible**: the list should show only the file's final committed state, no flicker. Cleanest via deferring the auto-applied emit to the settle point (which also matches "applied = verified") rather than emit-then-retract.
  - *Terminal-halt revert* (a file that exhausts its per-file budget and stays at last-good) → **show it** as reverted, paired with the halt message. The webview already has the rendering and an update-by-path hook to reuse: the `superseded` status (⊘, dimmed) at [MessageTurnActor.ts:2192](../../../media/actors/turn/MessageTurnActor.ts#L2192) + `updatePendingFileStatusByPath` at [VirtualListActor.ts:952](../../../media/actors/virtual-list/VirtualListActor.ts#L952). Add a distinct `reverted` status (or reuse `superseded`) and a backend emit on the terminal halt. (Surfaced by the Phase 2b-2 adversarial review.)

  **Status: deferred — behaviour decided, implementation not started, by design.** Risk/benefit doesn't justify building it yet: the defect is *low severity* (a stale dropdown row in the rare revert case — the file *content* is already correctly restored, the model is told, and the halt message explains it), while the fix is *moderate, cross-layer risk* — it touches the **happy-path** emission timing (every auto edit, not just reverts), the fiddly append-only `resolvedDiffs`/`_lastNotifiedDiffIndex` cursor, the actor-messaging boundary, **and** history-restore (the structural log feeds replay), and the webview layer has thin automated coverage. **If/when built, scope it minimally:** do **not** re-time the happy path (no defer-to-settle) — only add a status-flip on the rare terminal revert, reuse the existing ⊘ `superseded` rendering rather than a new status, and accept a brief flicker. Confines blast radius to the revert path. Until then this remains a documented known gap.

...
- **Unstated precondition, found false 2026-08-03 — the abort must actually reach the HTTP layer.** This ADR assumes `abortController.abort()` cancels the in-flight request, so the loop unwinds promptly and the awaited teardown is quick. That holds for `streamChat`, which forwards `options.signal`. It does **not** hold for `chat()` ([deepseekClient.ts:444](../../../src/deepseekClient.ts#L444)), which accepts `options.signal` and never passes it to the HTTP layer — so an aborted non-streaming probe stays parked in `await` until the response or the request timeout arrives. The one-loop-at-a-time guarantee survives (teardown is still awaited), but the *"tiny added latency on interrupt"* claimed above is wrong for that path: Stop during a `runToolLoop` probe can block the next send for up to the full timeout. Compounding it, [httpClient.ts:80](../../../src/utils/httpClient.ts#L80) selects `config?.signal ?? controller.signal`, so supplying a caller signal orphans the timeout controller — streaming requests therefore have **no** timeout at all. Fix is to combine both signals rather than choose one, and forward `options.signal` from `chat()`; tracked in CLAUDE.md § Active Bugs. Correctness of this ADR's decision is unaffected — only the latency consequence and an assumption it never stated. **Resolved 2026-08-03:** `httpClient` now takes `AbortSignal.any([caller, timeout])` (a caller abort keeps its `AbortError` identity; only the timeout controller's fire maps to `ECONNABORTED`), `chat()` forwards `options.signal`, and `streamChat`'s inactivity watchdog rejects on the zero-data branch it previously dead-ended in — so the abort reaches every request shape and a stream that hangs before its first byte self-terminates at 30s.
